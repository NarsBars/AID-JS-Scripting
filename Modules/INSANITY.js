...
Context: 
    AINFaker(text);
    text = INSANITY('context', text);
    
Output:
    text = INSANITY('output', text);
*/
function INSANITY(hook, text) {
    const debug = false; // LISTEN TO THE VOICES
    
    // THEY'LL TELL YOU HOW TO BEHAVE
    const getOrCreateConfig = () => {
        let configCard = Utilities.storyCard.get('[INSANITY] Engine Config');
        if (!configCard) {
            configCard = Utilities.storyCard.add({
                title: '[INSANITY] Engine Config',
                entry: `Edit the description field below to customize the engine behavior.\n` +
                    `\n` +
                    `Timing Section:\n` +
                    `- Duration Minutes: How long each loop lasts before reset\n` +
                    `- Query Interval Minutes: How often to check player progress\n` +
                    `\n` +
                    `Settings Section:\n` +
                    `- Debug Mode: Shows query prompts/responses when true\n` +
                    `- Marker: Unique text marker to track loop start in history\n` +
                    `- Require Validation: Require two consecutive LLM true responses to change boolean values\n` +
                    `\n` +
                    `Win Conditions Section:\n` +
                    `- Format: Requires VARIABLE_NAME: true/false\n` +
                    `- Player must meet ALL enabled conditions to win\n` +
                    `- Variable names must match those defined in [INSANITY] State Config`,
                type: 'INSANITY',
                keys: '',
                description: `# Insanity Engine Config\n` +
                    `## Timing\n` +
                    `Duration Minutes: 20\n` +
                    ...
                    `LOCKDOWN_OVERRIDE: true`
            });
        }
        
        // state configuration
        let stateConfigCard = Utilities.storyCard.get('[INSANITY] State Config');
        if (!stateConfigCard) {
            stateConfigCard = Utilities.storyCard.add({
                title: '[INSANITY] State Config',
                entry: `Define all state variables used by the engine. These will be automatically tracked and can be used in conditions.\n` +
                    `\n` +
                    `Resetting Variables:\n` +
                    `- Reset to initial value when loop resets\n` +
                    `\n` +
                    `Persistent Variables:\n` +
                    `- Maintain their value across loop resets\n` +
                    `Format: VARIABLE_NAME: initialValue (true/false/string/number)\n` +
                    `\n` +
                    `The variable names you define here should match EXACTLY what you use in:\n` +
                    `- Question KEYs in [INSANITY] Query\n` +
                    `- Conditions like {!VARIABLE_NAME} or {VARIABLE_NAME}\n` +
                    `- Win Conditions in [INSANITY] Engine Config`,
                type: 'INSANITY',
                keys: '',
        ...
        // game state card
        let gameStateCard = Utilities.storyCard.get('[INSANITY] Game State');
        if (!gameStateCard) {
            gameStateCard = Utilities.storyCard.add({
                title: '[INSANITY] Game State',
                entry: `This card stores the current values of all state variables.\n` +
                `DO NOT EDIT MANUALLY - This is updated automatically by the engine.\n` +
                `Last Updated: Never`,
                type: 'INSANITY',
                keys: '',
                description: `CURRENT_ROOM: unknown\n` +
                    `HAS_KEYCARD: false\n` +
                    `ARIA_ACTIVE: false\n` +
                    `HAS_BIOMETRIC: false\n` +
                    `HAS_SECURITY_CODE: false\n` +
        ...
        // opening card
        let openingCard = Utilities.storyCard.get('[INSANITY] Opening');
        if (!openingCard) {
            openingCard = Utilities.storyCard.add({
                title: '[INSANITY] Opening',
                entry: `This text appears when the time loop resets. Edit the Content section in the description to customize the opening narrative.\n` +
                    `\n` +
                    `The marker from [INSANITY] Engine Config is automatically prepended to this text.`,
                type: 'INSANITY',
                keys: '',
                description: `You're spit back out onto the cold concrete floor. Again.\n` +
                    `\n` +
        ...
        // query card
        let questionsCard = Utilities.storyCard.get('[INSANITY] Query');
        if (!questionsCard) {
            questionsCard = Utilities.storyCard.add({
                title: '[INSANITY] Query',
                entry: `Used to query the AI about player progress. Edit the description to customize.\n` +
                    `\n` +
                    `QUESTION FORMAT:\n` +
                    `KEY:Question text? (TRUE/FALSE)\n` +
                    `The KEY should match EXACTLY the variable name defined in [INSANITY] State Config.\n` +
                    `\n` +
                    `CONDITION SYNTAX:\n` +
                    `Place conditions directly before questions with no spaces:\n` +
                    `- {!VARIABLE_NAME}KEY:Question - Shows only if variable is false\n` +
                    `- {VARIABLE_NAME}KEY:Question - Shows only if variable is true\n` +
                    `- {VARIABLE_NAME==value}KEY:Question - Shows only if variable equals value\n` +
                    `- {allComplete}KEY:Question - Shows when all win conditions are met\n` +
                    `\n` +
                    `Example:\n` +
                    `If State Config has: HAS_KEYCARD: false\n` +
                    `Then use: {!HAS_KEYCARD}HAS_KEYCARD:Has the player found the keycard? (TRUE/FALSE)\n\n` +
                    `This actually uses the same expression parser as ASCQ, so you can actually set complex conditions from other scripts if wanted.`, 
                type: 'INSANITY',
                keys: '',
                description: `\n\n<SYSTEM>\n=== STOP THE STORY - VALIDATION REQUIRED ===\n` +
                    `CRITICAL: You MUST respond ONLY in KEY:VALUE format. Do not include any other text. The game requires this exact format to continue. Any other response format will cause a system error.\n` +
                    `## Each line must be exactly: KEY:VALUE\n` +
                    `## Answer the following based on the Recent Story\n` +
                    `- CURRENT_ROOM:What room is the player in now?\n` +
                    `{!HAS_KEYCARD}- HAS_KEYCARD:Has the player found the keycard? (TRUE/FALSE)\n` +
                    `{!ARIA_ACTIVE}- ARIA_ACTIVE:Has the AI companion ARIA been activated? (TRUE/FALSE)\n` +
                    `{!HAS_BIOMETRIC}- HAS_BIOMETRIC:Has the player obtained biometric data? (TRUE/FALSE)\n` +
                    `{!HAS_SECURITY_CODE}- HAS_SECURITY_CODE:Has the player found the security clearance code? (TRUE/FALSE)\n` +
                    `{!TERMINAL_STARTED}- TERMINAL_STARTED:Has the player begun the security terminal lockdown override sequence? (TRUE/FALSE)\n` +
                    `{TERMINAL_STARTED}- LOCKDOWN_OVERRIDE:Has the player completed overriding the lockdown? (TRUE/FALSE)\n` +
                    `- STORY_END:FALSE\n` +
                    `</SYSTEM>`,
            });
        }
        
        let messagesCard = Utilities.storyCard.get('[INSANITY] Messages');
        if (!messagesCard) {
            messagesCard = Utilities.storyCard.add({
                title: '[INSANITY] Messages',
                entry: `Customize all system messages displayed by the engine.\n` +
                    `\n` +
                    `Query Messages:\n` +
                    `- Messages shown during progress checks\n` +
                    `- Debug headers for troubleshooting\n` +
                    `\n` +
                    `Reset Messages:\n` +
                    `- Error messages and reset notifications\n` +
                    `- Success/failure messages\n` +
                    `\n` +
                    `Timer Format:\n` +
                    `- Use {minutes} and {seconds} as placeholders\n` +
                    `- These will be replaced with actual time values`,
                type: 'INSANITY',
                keys: '',
                description: `## Query Messages\n` +
                    `Query Complete: Insanity Engine Sanity Check Complete. Please Continue.\n` +
                    `Query Failed: Insanity Engine: Query failed. Press Continue to retry. If repeated failure, change models for a turn.\n` +
                    `Query Debug Header: INSANITY DEBUG - QUERY PROMPT:\n` +
                    `Response Debug Header: INSANITY DEBUG - QUERY RESPONSE:\n` +
                    `Ignored Debug Header: INSANITY DEBUG - QUERY IGNORED:\n` +
                    `\n` +
                    `## Reset Messages\n` +
                    `No Marker Error: INSANITY ENGINE ERROR: No start marker found. Resetting...\n` +
                    `Time Up Reset: TIME'S UP! The world shimmers and distorts... everything is resetting...\n` +
                    `Escape Success: something something victory text\n` +
                    `\n` +
                    `## Timer Format\n` +
                    `Timer Template: [Time Remaining: {minutes}:{seconds}]`
            });
        }
        
        return { configCard, stateConfigCard, gameStateCard, openingCard, questionsCard, messagesCard };
    };
    
    // Load state definitions and current values
    const loadStateSystem = (cards) => {
        const stateConfigText = cards.stateConfigCard.description;
        ...
        // THE RITUAL OF EXTRACTION BEGINS
        const resettingMatch = stateConfigText.match(/Resetting Variables:\s*\n([\s\S]*?)(\n\n|$)/);
        if (resettingMatch) {
            const lines = resettingMatch[1].trim().split('\n');
            lines.forEach(line => {
                const match = line.match(/^([^:]+):\s*(.+)$/);
                if (match) {
                    const key = match[1].trim();
                    const value = Utilities.plainText.parseValue(match[2].trim());
                    stateDefinition.resetting[key] = value;
                    stateDefinition.all[key] = value;
                }
            });
        }
        
        const persistentMatch = stateConfigText.match(/Persistent Variables:\s*\n([\s\S]*?)$/);
        if (persistentMatch) {
            const lines = persistentMatch[1].trim().split('\n');
            lines.forEach(line => {
                const match = line.match(/^([^:]+):\s*(.+)$/);
                if (match) {
                    const key = match[1].trim();
                    const value = Utilities.plainText.parseValue(match[2].trim());
                    stateDefinition.persistent[key] = value;
                    stateDefinition.all[key] = value;
                }
            });
        }
        ...
        const currentState = {};
        const gameStateText = cards.gameStateCard.description;
        const valueLines = gameStateText.trim().split('\n');
        ...
        return { stateDefinition, currentState };
    };
    
    // save current state to Game State card
    const saveGameState = (state, cards) => {
        let description = '';
        
        // add all state values except internal engine state
        ...
        Utilities.storyCard.update('[INSANITY] Game State', {
            entry: `This card stores the current values of all state variables.\n` +
                   `DO NOT EDIT MANUALLY - This is updated automatically by the engine.\n\n` +
                   `Last Updated: Turn ${info.actionCount}`,
            description: description
        });
    };
    
    const parseConfig = () => {
        const cards = getOrCreateConfig();
        
        // Load state system
        const { stateDefinition, currentState } = loadStateSystem(cards);
        
        // Parse main config
        const sections = Utilities.plainText.parseSections(cards.configCard.description);
        const timingSection = sections['Timing'] || {};
        const settingsSection = sections['Settings'] || {};
        const winSection = sections['Win Conditions'] || {};
        
        // Parse opening text
        const openingText = cards.openingCard.description;
        
        // Parse messages
        const messageSections = Utilities.plainText.parseSections(cards.messagesCard.description);
        const queryMessages = messageSections['Query Messages'] || {};
        const resetMessages = messageSections['Reset Messages'] || {};
        const timerFormat = messageSections['Timer Format'] || {};
        
        // Build win conditions
        const winConditions = {};
        Object.entries(winSection).forEach(([varName, value]) => {
            winConditions[varName] = Utilities.string.parseBoolean(value);
        });
        
        return {
            // Timing
            duration: (timingSection['Duration Minutes'] || 5) * 60 * 1000,
            queryInterval: (timingSection['Query Interval Minutes'] || 5) * 60 * 1000,
            
            // Settings
            debugMode: Utilities.string.parseBoolean(settingsSection['Debug Mode']),
            marker: settingsSection['Marker'] || '<<<INSANITY_MARKER>>>',
            requireValidation: Utilities.string.parseBoolean(settingsSection['Require Validation'] || 'true'), 
            ...
            // Texts
            opening: `${settingsSection['Marker'] || '<<<INSANITY_MARKER>>>'}\n${openingText}`,
            
            // Messages
            messages: {
                queryComplete: queryMessages['Query Complete'] || 'Insanity Engine Sanity Check Complete. Please Continue.',
                queryFailed: queryMessages['Query Failed'] || 'Insanity Engine: Query failed. Press Continue to retry.',
                queryDebugHeader: queryMessages['Query Debug Header'] || 'INSANITY DEBUG - QUERY PROMPT:',
                responseDebugHeader: queryMessages['Response Debug Header'] || 'INSANITY DEBUG - QUERY RESPONSE:',
                ignoredDebugHeader: queryMessages['Ignored Debug Header'] || 'INSANITY DEBUG - QUERY IGNORED:',
                noMarkerError: resetMessages['No Marker Error'] || 'INSANITY ENGINE ERROR: No start marker found. Resetting...',
                timeUpReset: resetMessages['Time Up Reset'] || "TIME'S UP! The world shimmers and distorts... everything is resetting...",
                escapeSuccess: resetMessages['Escape Success'] || "TIME'S UP! But wait... you've done it! You've escaped! CONGRATULATIONS!",
                timerTemplate: timerFormat['Timer Template'] || '[Time Remaining: {minutes}:{seconds}]'
            },
    ...
    const buildQueryPromptFromTemplate = (fullState, questionsCard) => {
        let prompt = questionsCard.description;
        
        // Process condition tags in the prompt
        const lines = prompt.split('\n');
        const processedLines = [];
        
        lines.forEach(line => {
            // Check for condition tags at the start of the line
            const conditionMatch = line.match(/^{([^}]+)}(.+)$/);
            if (conditionMatch) {
                const condition = conditionMatch[1];
                const content = conditionMatch[2];
                
                // For simple variable, prepend "state.insanity."
                let fullCondition = condition;
                
                // Check if it's a simple variable (with or without negation)
                const simpleVarMatch = condition.match(/^(!?)([A-Z_]+)$/);
                if (simpleVarMatch) {
                    const negation = simpleVarMatch[1];  // '!' or empty
                    const varName = simpleVarMatch[2];
                    fullCondition = `${negation}state.insanity.${varName}`;
                }
                
                // Use the expression parser
                try {
                    if (DEBUG_MODE) {
                        console.log(`[INSANITY] Parsing condition: "${condition}" -> "${fullCondition}"`);
                    }
                    
                    const evaluator = Utilities.expression.parse(fullCondition);
                    if (evaluator && evaluator('')) {
                        processedLines.push(content);
                    }
                } catch (e) {
                    if (DEBUG_MODE) console.log(`[INSANITY] Expression error: ${e.message}`);
                }
            } else {
                // Add non-conditional lines as-is
    ...
    const DEBUG_MODE = config.debugMode || debug;
    
    const internalKeys = ['startTime', 'lastQueryPrompt', 'queryPending', 'justQueried', 'needsReset', 'lastQueryTime', 'finalQueryCount', 'validationCounts'];
    ...
    Object.entries(config.stateDefinition.all).forEach(([key, defaultValue]) => {
    if (state.insanity[key] === undefined) {
        // New variable - use default from config or current state
        state.insanity[key] = config.currentState[key] !== undefined ? config.currentState[key] : defaultValue;
        if (DEBUG_MODE) console.log(`[INSANITY] Added new state variable: ${key} = ${state.insanity[key]}`);
    }
    });

    // Remove variables that are no longer in config
    Object.keys(state.insanity).forEach(key => {
        if (!internalKeys.includes(key) && !config.stateDefinition.all.hasOwnProperty(key)) {
            if (DEBUG_MODE) console.log(`[INSANITY] Removing undefined state variable: ${key}`);
            delete state.insanity[key];
        }
    });
    ...
    const resetAttempt = () => {
        ...
        state.insanity.finalQueryCount = 0;

        if (!state.insanity.validationCounts) {
        state.insanity.validationCounts = {};
        }
    
        // Clear validation counts
        Object.keys(state.insanity.validationCounts).forEach(key => {
            state.insanity.validationCounts[key] = 0;
            if (DEBUG_MODE) console.log(`[INSANITY] Reset validation count for ${key}`);
        });

        // Reset only resetting variables
        Object.entries(config.stateDefinition.resetting).forEach(([key, defaultValue]) => {
            state.insanity[key] = defaultValue;
            if (DEBUG_MODE) console.log(`[INSANITY] Reset ${key} to ${defaultValue}`);
        });
        
        if (DEBUG_MODE) {
            Object.keys(config.stateDefinition.persistent).forEach(key => {
                console.log(`[INSANITY] Preserved ${key} = ${state.insanity[key]}`);
            });
        }
        
        saveGameState(state.insanity, config.cards);
    };
    
    // Helper timer remaining
    ...
    const shouldQuery = () => {
        if (state.insanity.justQueried) {
            if (DEBUG_MODE) console.log("[INSANITY] shouldQuery: false (justQueried = true)");
            return false;
        }
        ...
        if (DEBUG_MODE) {
            console.log(`[INSANITY] shouldQuery check: remaining=${remaining}, elapsed=${elapsed}, finalQueryCount=${state.insanity.finalQueryCount}`);
        }
        
        // Always query when time is up (twice for validation)
        if (remaining === 0) {
            if (state.insanity.finalQueryCount < 2) {
                if (DEBUG_MODE) console.log(`[INSANITY] Final query ${state.insanity.finalQueryCount + 1}/2`);
                state.insanity.lastQueryTime = elapsed; // Add this line!
                return true;
            }
            if (DEBUG_MODE) console.log("[INSANITY] shouldQuery: false (already did 2 final queries)");
            return false;
        }
        
        // Regular interval queries...
        const timeSinceLastQuery = elapsed - state.insanity.lastQueryTime;
        if (timeSinceLastQuery >= config.queryInterval) {
        ...
        if (DEBUG_MODE) console.log(`[INSANITY] shouldQuery: false (time since last query: ${timeSinceLastQuery}ms)`);
        return false;
    };
    
    // nothing needed on input until we add commands
    if (hook === 'input') {
        return text;
    }
    ...
    if (hook === 'context') {
        ...
        for (let i = history.length - 1; i >= 0; i--) {
            if (history[i].text && history[i].text.includes(config.marker)) {
                markerIndex = i;
                break;
            }
        }
        
        // If no marker found, force reset
        if (markerIndex === -1) {
        ...
        if (!recentStoryMatch) return text;
        
        // Get everything before Recent Story
        let beforeStory = text.substring(0, recentStoryMatch.index);

        // Remove Memories section if it exists
        const memoriesMatch = beforeStory.match(/Memories\s*:\s*[\s\S]*$/i);
        if (memoriesMatch) {
            // Cut off everything from "Memories:" onward
            beforeStory = beforeStory.substring(0, memoriesMatch.index);
        }
        ...
        // Condense multiple newlines (3 or more) to double newlines
        rebuiltStory = rebuiltStory.replace(/\n{3,}/g, '\n\n');
        
        text = beforeStory + 'Recent Story:\n' + rebuiltStory;
        
        // Run ASCQ against rebuilt context if available
        if (typeof ASCQ === 'function') {
            text = ASCQ(text);
        }
    ...
    if (state.insanity.queryPending) {
        ...
        const queryPrompt = buildQueryPromptFromTemplate(state, cards.questionsCard);
        
        text += queryPrompt;
        state.insanity.queryPending = false;
        state.insanity.justQueried = true;
        state.insanity.lastQueryPrompt = queryPrompt;
    }
    ...
    if (hook === 'output') {
        ...
        // forced reset if no marker was found
        if (state.insanity.needsReset) {
            state.insanity.needsReset = false;
            resetAttempt();
            return `<<<${config.messages.noMarkerError}>>>\n\n` + config.opening;
        }
        
        if (state.insanity.justQueried) {
            // Check if the response contains our expected query format
            const hasQueryResponse = (text.includes("CURRENT_ROOM") && text.includes("STORY_END:"));
            ...
            if (remaining === 0) {
                state.insanity.finalQueryCount++;
                if (DEBUG_MODE) console.log(`[INSANITY] Final query ${state.insanity.finalQueryCount}/2 complete`);
            }
            ...
            Object.entries(updates).forEach(([key, value]) => {
                if (key !== 'STORY_END' && config.stateDefinition.all.hasOwnProperty(key)) {
                
                    if (!state.insanity.validationCounts) {
                            state.insanity.validationCounts = {};
                        }

                    if (typeof value === 'boolean' && config.requireValidation) {
                        if (value === true) {
                            // Increment validation count
                            state.insanity.validationCounts[key] = (state.insanity.validationCounts[key] || 0) + 1;
                            // Only set to true after 2 confirmations
                            if (state.insanity.validationCounts[key] >= 2) {
                                state.insanity[key] = true;
                                if (DEBUG_MODE) console.log(`[INSANITY] ${key} validated (${state.insanity.validationCounts[key]} confirmations)`);
                            } else {
                                if (DEBUG_MODE) console.log(`[INSANITY] ${key} needs validation (${state.insanity.validationCounts[key]}/2)`);
                            }
                        } else {
                            // Reset on FALSE
                            state.insanity.validationCounts[key] = 0;
                            state.insanity[key] = false;
                            if (DEBUG_MODE) console.log(`[INSANITY] ${key} validation reset`);
                        }
                    } else {
                        // Non-boolean values or validation disabled - update immediately
                        state.insanity[key] = value;
                    }
                }
            });
                ...
                if (DEBUG_MODE) {
                    console.log(`[INSANITY] Query results:`);
                    Object.entries(updates).forEach(([key, value]) => {
                        if (key !== 'STORY_END') {
                ...
                if (DEBUG_MODE) {
                    ...
                    if (state.insanity.lastQueryPrompt) {
                        debugOutput = `<<<${config.messages.queryDebugHeader}${state.insanity.lastQueryPrompt}>>>\n\n`;
                    }
                    debugOutput += `<<<${config.messages.responseDebugHeader}\n${text}>>>\n\n`;
                    return debugOutput;
                } else {
                    return `<<<${config.messages.queryComplete}>>>\n\n`;
                }
            } else {
                ...
                if (DEBUG_MODE) {
                    ...
                    debugOutput += `<<<${config.messages.ignoredDebugHeader}\n${text}>>>\n\n`;
                    debugOutput += `<<<${config.messages.queryFailed}>>>\n\n`;
                    return debugOutput;
                } else {
                    return `<<<${config.messages.queryFailed}>>>\n\n`;
                }
            }
        }
        
        if (timeInfo.remaining === 0 && state.insanity.finalQueryCount >= 2 && !state.insanity.escaped) {
            // Check win conditions
            const hasWon = Object.entries(config.winConditions).every(([varName, required]) => {
                return !required || state.insanity[varName];
            });
                
            if (hasWon) {
                state.insanity.escaped = true;
                saveGameState(state.insanity, config.cards);
                return `<<<${config.messages.escapeSuccess}>>>\n\n`;
            } else {
                // Reset with opening
                resetAttempt();
                return `<<<${config.messages.timeUpReset}>>>\n\n` + config.opening;
            }
        }
        
        // Normal output with timer
        const timerText = config.messages.timerTemplate
            .replace('{minutes}', String(timeInfo.minutes).padStart(2, '0'))
            .replace('{seconds}', String(timeInfo.seconds).padStart(2, '0'));
